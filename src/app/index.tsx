    ...
    <ScrollView style={globalStyles.container}>
      ...
      <View style={{justifyContent: 'center', minHeight: height * 0.35, }}> 
        ...
        <View style={{flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between'}}>
          ...
          <Image source={require('../../assets/weatherImages/sun.png')} style={{ width: 50, height: 50, tintColor: '#c8753a', marginTop: height * 0.05}} />
        </View>
        <Text style={globalStyles.sectionTitle}>{weatherData?.weather?.[0]?.description}</Text>
      </View> 
      <View style={{flexDirection: 'row', gap:20}}>
        <Text style={globalStyles.feels}><Text style={{color: '#000000'}}>{weatherData?.main?.feels_like.toFixed(0)}°</Text><Text >feels</Text></Text>
        <Text style={globalStyles.feels}><Text style={{color: '#000000'}}>{weatherData?.main?.humidity.toFixed(0)}% </Text><Text>hum</Text></Text>
        <Text style={globalStyles.feels}><Text style={{color: '#000000'}}>{weatherData?.wind?.speed.toFixed(1)} </Text><Text>wind</Text></Text>
      </View>
